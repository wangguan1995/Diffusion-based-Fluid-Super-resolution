...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        x = torch.nn.functional.interpolate(
            x, scale_factor=[1,2,2], mode="nearest")
        if self.with_conv:
            x = self.conv(x)
        return x
    ...
    def forward(self, x):
        if self.with_conv:
            ...
            for i in range(x.size(2)):
                x_list.append(torch.nn.functional.pad(x[:,:,i], pad, mode="circular"))
...
class AttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # b, c,thw (thw of q) h_[b,c,j] = sum_i v[b,c,i] w_[b,i,j]
        h_ = torch.bmm(v, w_)
...
class Model_Spatial_Temperal(nn.Module):
    ...
    def forward(self, x, t):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks+1):
                hs_pop = hs.pop()
                input_tensor = torch.cat([h, hs_pop], dim=1)
